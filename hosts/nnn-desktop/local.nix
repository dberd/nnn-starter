...
# modules/nixos/vpn.nix for how those are handled.
{
  ...
  #
  # flake.nix derives `gameOutput` from this attrset — the biggest panel by
  # pixel count — so gaming.nix and the gamescope window rule in niri.nix both
  # follow a monitor swap without anything else being edited here.
  monitors = {
    "HDMI-A-2" = {
